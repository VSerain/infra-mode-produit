# Accès à l'infrastructure Kubernetes pour un projet en mode produit (Lean Startup/BetaGouv)
Document visant à expliquer les besoins d'accès à l'infra Kubernetes dans un produit (Mode produit, Lean Startup) ainsi que les justifications qui expliquent ces besoins.
...
Nous comprenons que les équipes SI ont des **obligations historiques et des contraintes fortes** :

* Sécurité des données et conformité (RGPD, normes internes, bonne connaissance des enjeux liés à la cybersécurité).
* Homologation de sécurité des applications et des services.
* Prévention des risques liés à une intrusion potentielle ou à la propagation d'un virus dans le réseau de l'administration.
* Stabilité et fiabilité des systèmes, y compris critiques.
* Processus de validation et de contrôle hérités.

Notre objectif est de **trouver ensemble** comment fonctionner en **agile** grâce à un **accès ciblé et encadré** à certaines couches de l’infra, **sans compromettre la sécurité et la stabilité des systèmes, et en s'intégrant dans les processus DSI**.
...
* **Proposer un cadre fonctionnel respectant les contraintes DSI** : traçabilité, principe de moindre privilège, audit, afin de collaborer efficacement.
...
Les modes agiles s'appuient sur des logiques de développements itératifs, généralement lors de sprints courts d'une à deux semaines. Les évolutions sont donc rapides et potentiellement nombreuses. Dans une approche centrée sur l'utilisateur, la pertinence des fonctionnalités développées est évaluée à l'aide de tests en conditions réelles ou par des retours (feedbacks) de la communauté. 

C'est pourquoi un projet en mode produit agile (Lean Startup, DevOps) **nécessite une infra tout aussi agile** pour éviter des **goulots d’étranglement** qui en annulent les bénéfices.
...
* **Impact utilisateur** : Les utilisateurs finaux sont également pénalisés, car ils ne bénéficient pas des corrections de bugs et des nouvelles fonctionnalités aussi rapidement qu'il serait possible de les livrer.
...
* **Exemple** : Donner aux devs un accès **ciblé et contrôlé** aux couches applicatives (logs, monitoring, déploiement en staging), tout en gardant les couches basses (hardware, VM) sous le contrôle exclusif de la DSI.
...
| **Services tiers** (FTP, APIs externes, mail)    | ✅ **Oui** (visibilité, redémarrage)                    | Diagnostiquer des pannes et relancer un service. *Note : Cela implique aussi que certaines machines puissent être exposées sur Internet et non pas seulement accessibles en réseau interne (ex: réseau Cerema).* | Optionnel (peut être contourné temporairement) |
...
| **Sauvegarde** (dump, snapshot)                  | ✅ **Oui** (lecture)                            | Vérifier l'existence de sauvegardes quotidiennes, prise en compte rapide d'une restauration en cas de version corrompue. | **Critique**                                   |
| **Certificats** (DNS, HTTPS)                     | ✅ **Oui** (lecture, demande d'action)                            | Vérifier la validité, anticiper la demande de remplacement d'un certificat expiré. | **Critique**                                   |

**→ Résumé** :

* **Critique** : Conteneurs, logs, monitoring, bases de données (non-prod), déploiement en staging/prod via pipeline.
* **Optionnel** : Services tiers (peut être géré par la DSI en cas de blocage).
...
* **Solution** : Donner un accès **cadré et sécurisé** limite ce risque.
...
## 5. Quel cadre fonctionnel pour respecter tous ces enjeux ?
...
### 5.3. Formation et collaboration

* **Former les devs** aux bonnes pratiques de sécurité et aux contraintes réseau de l'administration (ex. : ne jamais modifier la prod directement).
* **Désigner un "ambassadeur DevOps"** dans l’équipe pour faciliter le dialogue continu avec la DSI.
...
| 3     | Formation des devs aux bonnes pratiques de sécurité et contraintes SI.   | SI                  |
...
* **Maîtrise des risques** (traçabilité, principe de moindre privilège, respect de l'homologation).
* **Réduction du shadow IT** (moins de contournements).
* **Meilleure collaboration** avec les équipes produit.

### 8.2. Pour les développeurs et les utilisateurs

* **Autonomie et réactivité** (moins de blocages).
* **Meilleure qualité** (diagnostics rapides, déploiements fiables pour les utilisateurs finaux).
...
* **L’agilité ne se limite pas au développement** : sans infra adaptée, les gains sont annulés et l'utilisateur final en pâtit.
* **L’hybridation est possible** : accès ciblé aux couches applicatives + contrôle de la DSI sur la sécurité globale et les couches basses.
* **L’objectif est commun** : livrer de la valeur rapidement, dans un cadre sécurisé et maîtrisé.
